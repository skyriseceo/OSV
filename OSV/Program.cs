...
try
{
    ...
    builder.Services.AddScoped<OSV.Attributes.CalComSignatureAuthFilter>();// Services
    ...
    // (2) <--- إضافة
    // ------------------------------------
    // (مهم جداً) لازم السطر ده عشان [RetellSignatureAuth] يشتغل
    // بيسمح بقراءة الـ Request Body أكتر من مرة (مرة للـ Auth ومرة للـ Model Binding)
    ...
    // (3) <--- إضافة
    // ------------------------------------
    // السطر ده هو اللي بيشغل الـ [ApiController] والـ Attribute Routing
    app.MapControllers();
    // -----------------------------------
    app.MapRazorPages();
